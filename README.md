...
<body>
    ...
    <div class="background">
        <div class="card" id="login-card">
            <h1 class="title">Hey pretty, what's the magic word? ❤</h1>
        ...
        <div class="welcome-screen" id="welcome-screen">
            <h2 style="color: #d81b60; font-size: 32px;">Hello Hanyy 🔥</h2>
            <p style="color: #ad1457;">I hope you enjoy your stay 🥰</p>
            <button class="start-button" id="start-exploring-button">Start Exploring ✨</button>
        </div>

        <div class="main-content" id="main-content">
            <header class="main-header">
                <h1>For Haniy ireng 💞</h1>
                <p>A special place just for you</p>
                <nav class="navigation">
                    <a href="#" class="nav-item" data-target="love-zone-section">Love Zone 🏰</a>
                    <a href="#" class="nav-item" data-target="music-section">Music 🎶</a>
                    <a href="#" class="nav-item" data-target="gallery-section">Gallery 🖼️</a>
                    <a href="#" class="nav-item" data-target="notes-section">Notes 📝</a>
                </nav>
            </header>

            <main class="content-area">
                <div class="content-section" id="love-zone-section">
                    <h2 style="color: #e91e63; margin-bottom: 25px;">Our Love Wishlist ✨</h2>
                ...
                <div class="content-section" id="music-section">
                    <h2 style="color: #e91e63; margin-bottom: 25px;">Lagu Kita 🎶</h2>
                    <div class="music-player-card">
                         <label for="music-upload" class="custom-file-upload">Upload Lagu 🎵</label>
                ...
                <div class="content-section" id="gallery-section">
                    <h2 style="color: #e91e63; margin-bottom: 20px;">Our Memories 🖼️</h2>
                    <div class="upload-wrapper">
                        <label for="photo-upload" class="custom-file-upload">Tambah Foto 📸</label>
                ...
                <div class="content-section" id="notes-section">
                    <h2 style="color: #e91e63; margin-bottom: 20px;">Love Board 📝</h2>
                    <div class="note-input-container">
                        ...
                        <button class="custom-file-upload" id="post-note-btn">Kirim Pesan ✨</button>
                    </div>
    ...
    <script>
        ...
        let uploadedPhotos = JSON.parse(localStorage.getItem('saved_photos')) || [];
        let uploadedSongs = JSON.parse(localStorage.getItem('saved_songs')) || [];
        let notes = JSON.parse(localStorage.getItem('saved_notes')) || [];
        let wishlist = JSON.parse(localStorage.getItem('saved_wishlist')) || [];
        ...
        enterButton.onclick = () => {
            if (magicInput.value.toLowerCase().trim() === MAGIC_WORD) {
                ...
                document.getElementById('error-message').textContent = 'Kata sandi salah! 😊';
            }
        };
        ...
        function syncStorage() {
            ...
            localStorage.setItem('saved_wishlist', JSON.stringify(wishlist));
            localStorage.setItem('saved_photos', JSON.stringify(uploadedPhotos));
            localStorage.setItem('saved_songs', JSON.stringify(uploadedSongs));
        }

        // --- LOVE ZONE / WISHLIST LOGIC ---
        ...
        function renderWishlist() {
            ...
            container.innerHTML = '';
            const sortedWishlist = [...wishlist].sort((a, b) => a.isCompleted - b.isCompleted);

            sortedWishlist.forEach(item => {
                const div = document.createElement('div');
                div.className = `wish-item ${item.isCompleted ? 'completed' : ''}`;
                div.innerHTML = `
                    <input type="checkbox" class="wish-checkbox" ${item.isCompleted ? 'checked' : ''} onchange="toggleWish(${item.id})">
                    <span class="wish-text">${item.text}</span>
                    <button class="delete-wish-btn" onclick="deleteWish(${item.id})">×</button>
                `;
        ...
        document.getElementById('post-note-btn').onclick = () => {
            ...
            if (text) {
                notes.push({ id: Date.now(), text, color: selectedNoteColor, isPinned: false });
        ...
        function renderNotes() {
            ...
            sortedNotes.forEach(n => {
                ...
                d.innerHTML = `
                    <div class="pin-icon" onclick="togglePin(${n.id})">${n.isPinned ? '📌' : '📍'}</div>
                    <div class="note-content">${n.text}</div>
                    <div class="note-actions"><button class="action-btn" onclick="deleteNote(${n.id})">🗑️</button></div>
                `;
                board.appendChild(d);
            });
        }
            ...
            uploadedPhotos.forEach(p => {
                ...
                wrapper.innerHTML = `
                    <div class="photo-card" style="--orig-rot: ${p.rot}deg; transform: rotate(${p.rot}deg)">
                        ...
                        <img src="${p.src}">
                        <button onclick="deletePhoto(${p.id})" style="position:absolute; bottom:5px; right:5px; border:none; background:none; cursor:pointer; font-size:12px;">🗑️</button>
                    </div>
                `;
        ...
        document.getElementById('music-upload').onchange = (e) => {
            ...
            if (file) {
                const reader = new FileReader();
                reader.onload = (ev) => {
                    uploadedSongs.push({ id: Date.now(), title: file.name, data: ev.target.result });
                    syncStorage();
                    renderSongs();
                };
                reader.readAsDataURL(file);
        ...
        function renderSongs() {
            ...
            list.innerHTML = '';
            uploadedSongs.forEach(s => {
                const item = document.createElement('div');
                item.className = 'song-card';
                item.innerHTML = `
                    <span style="cursor:pointer" onclick="playSong('${s.id}')">🎵 ${s.title}</span>
                    <button onclick="deleteSong(${s.id})" style="border:none;background:none;cursor:pointer">🗑️</button>
                `;
        ...
        window.deleteSong = (id) => { 
            uploadedSongs = uploadedSongs.filter(x => x.id !== id); 
            syncStorage();
            renderSongs(); 
        };
